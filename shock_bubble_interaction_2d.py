...
.. math::
    ...
    (\rho u)_t + (\rho u^2 + p)_z + (\rho uv)_r & = -\rho u v / r \\
    (\rho v)_t + (\rho uv)_z + (\rho v^2 + p)_r & = - \rho v^2 / r \\
    E_t + (u (E + p) )_z + (v (E + p))_r & = - (E + p) v / r.
...
def setup(nx=[320,80],kernel_language='Fortran',solver_type='sharpclaw',time_integrator='SSP104',\
        lmm_steps=None,cfl=None,lim_type=2,limiter=4,dt_variable=True,dt_initial=None,\
        outdir='./_output',disable_output=False,paramtrs=Parameters()):
    ...
    if solver_type=='sharpclaw':
        solver = overridden_fun.set_solver(pyclaw.SharpClawSolver2D,riemann.euler_5wave_2D,
                claw_package=claw_package)
        ...
        solver = overridden_fun.set_solver(pyclaw.ClawSolver2D,riemann.euler_5wave_2D,
                claw_package=claw_package)
